...
intro_subs_did = {
    ...
    2: "Hey!<br>The next song is called:<br>'{song_title}'.|I composed it for:<br><span style='text-transform:uppercase;'>{user_name}</span>|You wanted a song with some {song_style} notes, right?|Let’s see how it turns out.<br>Haha.<br>Kisses, <span style='text-transform:uppercase;'>{user_name}</span>.",
    ...
    4: "Dear <span style='text-transform:uppercase;'>{user_name}</span>,<br>I’ve worked hard to complete the writing of your song.|It's called:<br>'{song_title}'|It has notes of<br>{song_style} music with lyrics that I hope will resonate with you.|Watch out, <span style='text-transform:uppercase;'>{user_name}</span>!<br><br>I'm starting the machine now!",
    5: "The next song is for you,<br><span style='text-transform:uppercase;'>{user_name}</span>!|It's called: '{song_title}'. |I wrote it as best as I could, but I was a bit rushed.|Don’t be mad<br>if it’s bad.<br>Kisses, <span style='text-transform:uppercase;'>{user_name}</span>.",
    6: "I don’t mean to brag, but I think the next song is genius!|I wrote it for:<br><span style='text-transform:uppercase;'>{user_name}</span>|It's called:<br>'{song_title}.'|Honestly, I think it’s pretty good.<br><br><span style='text-transform:uppercase;'>{user_name}</span>, get ready!",
    7: "It wasn’t easy,<br>but I finished writing<br>'{song_title}'|I wrote it<br>especially for<br><span style='text-transform:uppercase;'>{user_name}</span>.|A song that's a bit {song_style}, I hope will hit the mark.|Alright, let’s give it a listen!"
}
