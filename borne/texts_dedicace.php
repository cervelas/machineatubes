...
$texts_dedicace_eng = array(
    "And there you go, another hit composed quickly and efficiently! It's called: ".$_SESSION['song_title']." and it was written especially for: ".$_SESSION['user_name'].". I’ve tried to match the ".$humeur[$_SESSION['song_style']] . "energy you were into. Shall we listen? Here we go!",
    ...
    "Hello! The next song is called: ".$_SESSION['song_title'].". I composed it for: ".$_SESSION['user_name'].", who seemed to want a song with some ".$humeur[$_SESSION['song_style']]." notes. Let’s see how it turns out. Ha ha ha. Kisses, ".$_SESSION['user_name'].".",
    "All set, I'm ready! Watch out, ".$_SESSION['user_name'].", the next one is for you! It's called: ".$_SESSION['song_title'].". To write this beautiful song, I combined your wishes with my inspirations, set to a music in a ".$humeur[$_SESSION['song_style']]." style. Now we just have to hope you like it. 3, 2, 1, go!",
    
    "Dear ".$_SESSION['user_name'].", I’ve worked hard to complete the writing of ".$_SESSION['song_title'].", just as you envisioned. Notes of ".$humeur[$_SESSION['song_style']]." music with lyrics that I hope will resonate with you. Here it is. Enjoy! Watch out, ".$_SESSION['user_name'].", I'm starting the machine now. Bye-bye.",
    ...
    "Well, it wasn’t easy, but I’ve finally finished writing ".$_SESSION['song_title'].". Especially for ".$_SESSION['user_name'].". A song that's a bit ".$humeur[$_SESSION['song_style']].", which I hope will hit the mark. Let’s give it a listen."
);
